...
(function (root) {
  ...
  // v8 — บังคับ crop แต่ละ scene ให้เหลือเฉพาะ panel ตัวเอง ห้ามโชว์ grid
  // เต็มแผ่น". Do not paraphrase — confirmed byte-for-byte against the
  // spec source via a Python diff against the exact old/new sentence
  // pair the spec gave (same discipline as v6->v7, since v8 only
  // inserts one CRITICAL clause into an existing sentence in Section
  // 3). Toey found live-generated videos were showing the raw
  // uncropped 5-panel storyboard grid itself as a scene (especially
  // Scene 1), instead of that scene's own single panel filling the
  // frame — v8 inserts an explicit "CRITICAL — per-scene crop"
  // instruction requiring every scene to show ONLY its own panel,
  // tightly cropped edge-to-edge, with Scene 1 called out specifically
  // as needing to already start cropped into panel 1's hero framing
  // rather than the raw grid image. Every other sentence — Sections 1,
  // 2, the rest of Section 3 including v7's text-lock CRITICAL
  // paragraph — confirmed byte-identical to v7 via the same diff. All
  // 3 section headings unchanged — no parse-code impact, same
  // reasoning established since v5.
