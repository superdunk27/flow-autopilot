...
(function (root) {
  ...
  // v9 — เปลี่ยน scene-1-crop instruction จากคำสั่งห้ามเป็น positive
  // framing". Do not paraphrase — confirmed via 3 independent checks
  // against the spec's exact old/new paragraph pair: (1) verbatim
  // match (new paragraph present, old one absent), (2) splice-
  // reconstruction (old template minus old paragraph plus new
  // paragraph, at the same index, equals the actual new template
  // byte-for-byte), (3) length arithmetic (the template's length delta
  // equals the paragraph pair's length delta exactly, +120 chars).
  // v8's "CRITICAL — per-scene crop" paragraph was phrased as a
  // prohibition (what NOT to show — the full grid); v9 replaces it
  // with positive framing instead — describing each scene as if it
  // were handed only that one panel's photo already cropped/enlarged/
  // centered, with the grid/borders/gutters/badges simply absent from
  // what the model is asked to imagine, rather than telling it what to
  // avoid showing. v7's text-lock CRITICAL paragraph (kept working,
  // untouched per Toey's explicit instruction) and every other
  // sentence in Sections 1, 2, and the rest of Section 3 confirmed
  // byte-identical to v8. All 3 section headings unchanged — no
